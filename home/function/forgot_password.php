...
if (isset($_POST['email'])) {
    ...
    if ($result->num_rows == 1) {
        // Generate a raw token for password reset
        $rawToken = bin2hex(random_bytes(50));  // Generate raw token
        $hashedToken = password_hash($rawToken, PASSWORD_DEFAULT);  // Hash the token to store in the database
        ...
        try {
            ...
            $resetLink = 'https://danrosefishing.com/home/function/reset_password.php?token=' . urlencode($rawToken) . '&email=' . urlencode($email); // Send raw token
